...
async function validateSubscription() {
    const eventPath = process_1.env.GITHUB_EVENT_PATH;
    let repoPrivate;
    ...
    const action = process_1.env.GITHUB_ACTION_REPOSITORY;
    ...
    if (repoPrivate === false)
        return;
    const serverUrl = process_1.env.GITHUB_SERVER_URL || 'https://github.com';
    const body = { action: action || '' };
    if (serverUrl !== 'https://github.com')
        body.ghes_server = serverUrl;
    const repository = process_1.env.GITHUB_REPOSITORY ?? '';
    try {
        await axios_1.default.post(`https://agent.api.stepsecurity.io/v1/github/${repository}/actions/maintained-actions-subscription`, body, { timeout: 3000 });
    }
    catch (error) {
...
async function run() {
    try {
        ...
        if (text.length > MAX_TEXT_LENGTH) {
            core.setFailed(`Input 'text' exceeds maximum length of ${MAX_TEXT_LENGTH} characters`);
            return;
        }
        if (regex.length > MAX_REGEX_LENGTH) {
            core.setFailed(`Input 'regex' exceeds maximum length of ${MAX_REGEX_LENGTH} characters`);
            return;
        }
        const re = new RegExp(regex, flags);
        const result = vm.runInNewContext('re.exec(text)', { re, text }, { timeout: REGEX_TIMEOUT_MS });
        if (result) {
        ...
        if (error instanceof Error && error.message.includes('timed out')) {
            core.setFailed('Regex execution timed out — the pattern may cause catastrophic backtracking');
            return;
        }
